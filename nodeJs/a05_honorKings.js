...
var masterMap = {}
// 爬取图片存放路径 
const IMG_DIR = "../00_illustration/09_" + TBL_NAME + "/"
// 保存json文件
const JSON_FILE = "./resources/data/data_" + TBL_NAME + ".json"
// 基本域名
const BASE_HOST = "https://pvp.qq.com/web201605/"
const IMG_HOST = ""
// 下载开关
...
async function main() {
    start()
    // 取得一览列表
    let listUrl = BASE_HOST + "herolist.shtml"
    // 发送请求,将响应内容写入文件
    const $ = await util.getTargetElementByAxios(listUrl, "ul.herolist,clearfix", "templateList.html", true)
    // 开始取得一览 
...
function getListInfo($) {
    let dataList = []
    $("li>a").each((i, aEle) => {
        console.log($(aEle).find("img")[0])
        // 名称
        let name = $($(aEle).find("img")[0]).html()
...
async function getDetailInfo(currentData) {
    ...
    // 登录DB
    if (DB_FLAG) {
        await dbUtil.insetUpdateData(dbModel, currentData, { "no": currentData[PK_ATTR] })
    }
    // 写入json
